...
export default function EditPage() {
  ...
  //   update data
  const editUser = async (npm: string, nama: string, jurusan: string) => {
    if (npm == "" || nama == "" || jurusan == "") {
    ...
    const checkData = db.getFirstSync(
      `SELECT npm FROM tb_mahasiswa WHERE npm = '${npm}' AND id != '${id}'`
    );
    if (checkData) {
      setSnackbarVisible(true);
      setSnackbarMessage("Data Gagal Diubah !");
      setSnackbarError(1);
      return;
    }
    ...
    // if (!name) return;
    db.execSync(
      `UPDATE tb_mahasiswa SET npm ='${npm}', nama = '${nama}',jurusan = '${jurusan}',foto = '${fileName}' WHERE id = '${id}'`
    );
    setSnackbarVisible(true);
    setSnackbarMessage("Data Berhasil Diubah");
    setSnackbarError(0);
  ...
  const deleteImage = async () => {
    const location = `${FileSystem.documentDirectory}${form.foto}`;
    if (!form.foto) return;
            ...
            <Image
              source={
                imageUri
                  ? { uri: imageUri }
                  : form.foto != "" &&
                      `${FileSystem.documentDirectory}${form.foto}`
        ...
        <View style={{ margin: 20 }}>
          ...
          <View style={{ marginTop: 50, flexDirection: "row" }}>
            <Button
              icon="check"
              mode="contained"
              onPress={() => editUser(form.npm, form.nama, form.jurusan)}
